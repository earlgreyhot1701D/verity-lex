...
const policyUrl = "https://court.example.gov/full-policy.pdf";
const indexUrl = "https://court.example.gov/policies";
const mergeActions = [
  { reason: "find full policy", action: "discover", query: "full policy" },
  { reason: "fetch full policy", action: "fetch", url: policyUrl },
  { reason: "extract full policy", action: "extract", url: policyUrl },
  { reason: "find index", action: "discover", query: "policy index" },
  { reason: "fetch index", action: "fetch", url: indexUrl },
  { reason: "extract index", action: "extract", url: indexUrl },
  { reason: "finish", action: "finish" },
];
let mergeModelCalls = 0;
const mergeScan = await runAgentLoop({
  institution: "Merge Court",
  targetDomain: "court.example.gov",
  registry,
  model: {
    async complete() {
      return JSON.stringify(mergeActions[mergeModelCalls++] ?? mergeActions.at(-1));
    },
  },
  limits: { maxIterations: 7, maxFetches: 3, timeoutMs: 1 },
  tools: {
    async discover(query) {
      const url = query === "full policy" ? policyUrl : indexUrl;
      return [{ title: "Policy", url, snippet: "official" }];
    },
    async fetch(url) {
      return { ok: true, url, text: "<UNTRUSTED_DOCUMENT_TEXT>x</UNTRUSTED_DOCUMENT_TEXT>", fetchedAt: "2026-07-19T00:00:00.000Z" };
    },
    async extract(url) {
      return url === policyUrl
        ? { sourceUrl: url, signals: [{ artifactId: "genai_policy", present: true, quotedSpan: "complete policy", elementsPresent: registry.artifacts[0].elements }] }
        : { sourceUrl: url, signals: [{ artifactId: "genai_policy", present: true, quotedSpan: "Generative AI Policy" }] };
    },
    evaluate(institution, signals, loadedRegistry) { return evaluate(institution, signals, loadedRegistry); },
  },
});

assert.equal(mergeScan.score.byDimension.A, 35, "a later shallow signal cannot erase complete policy coverage");
assert.equal(mergeScan.findings.find((finding) => finding.artifactId === "genai_policy")?.source?.url, policyUrl, "the strongest evidence source is retained");
